...
def DeferralRPS(
    ...
    if repaymentMethod == "Deferral":
        ...
        disbursementDateRPS = date(DisbursementYear, DisbursementMonth, DisbursementDay)

        # Create the first row of the RPS to show the disbursement of the loan
        rps['SNo'].append(0)
        rps['Date'].append(disbursementDateRPS)
        rps['Days'].append(0)
        rps['EMI'].append(0)
        ...
        if GracePeriodMonths > 0:
            ...
            while rps["OutstandingPrincipal"][-1] != 0:
                rps['SNo'].append(i)
                rps['Date'].append(date(FirstEMIYear + ((FirstEMIMonth + (i-1))//12), ((FirstEMIMonth + (i-1)) % 12) + 1, PayDay))
                rps['Days'].append((rps['Date'][-1] - rps['Date'][-2]).days)
                rps['EMI'].append(EMI)
                rps['ProfitAmount'].append(round(rps['OutstandingPrincipal'][-1] * ProfitRate / 360 * rps['Days'][-1],3))
                rps['TakafulAmount'].append(round(rps['OutstandingPrincipal'][-1] * TakafulFactor,3))
            ...
            raise AttributeError("Grace Period must be a non-negative integer.")

    
    else:
        raise AttributeError("This function supports 'Deferral' as the only repayment method.")
    
    rps['Date'] = [i.strftime("%Y-%m-%d") for i in rps['Date']]
